...
mutable struct Iter{AO}
    ...
    lazy_axes::Array{Bool, 1}    # true for axes selected with `:` (Colon)
end
...
"""
function Iter(ao::AO; lazy_axes=Bool[]) where {AO}
    # Obtain diemension of the array
    nd = ndims(ao)

    lazy_axes = isempty(lazy_axes) ? fill(false, nd) : lazy_axes

    # Intialize default values
    it = _init_iter(ao, nd, lazy_axes)
...
function _init_iter(ao::AO, nd::Integer, lazy_axes) where {AO}
...
function _iter_next!(it::Iter)
    ...
    for d in nd:-1:1
         ...
         if it.lazy_axes[d]
            # lazy full-axis: coordinate does not increment
            continue
        end
...
function value(it::Iter)
    if it.contiguous
        return item(it.ao, it.index)
    end
    return it.ao[_offset(it, length(it.coordinates))]
end
