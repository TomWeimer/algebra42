...
function Base.getindex(prep_shape::PaddedShape{OriginalDimension, expectedDimension}, i::Int) where {
    OriginalDimension, 
    expectedDimension
}
...
Base.length(padded_shape::PaddedShape{OriginalDimension, expectedDimension}) where {OriginalDimension, expectedDimension} = (
    expectedDimension
)

Base.axes(padded_shape::PaddedShape{OriginalDimension, PaddedDimension}) where {OriginalDimension, PaddedDimension} = (
    map(Base.OneTo, padded_shape)
)
