...
Base.IndexStyle(::CartesianIndices_42)         = IndexCartesian()
...
# LinearIndices
_getElement(a::CartesianIndices42C, i::Int) = _getElement(a, from_linear_to_cartesian(size(a), i))

# LinearIndices Row major
_getElement(a::CartesianIndices42R, i::Int) = _getElement(a, from_linear_to_cartesian(size(a), i, order=RowOrder))

# CartesianIndex
_getElement(a::CartesianIndices_42{N},  I::AllCartesianIndex{N}) where N = CartesianIndex_42( map((range, i) -> range[i], a.indices, I) )

# 'Normal' indices
_getElement(a::CartesianIndices_42{N},  indices::NTuple{N, Index}) where N = CartesianIndex_42( map((range, i) -> range[i], a.indices, indices) )
...
_setElement!(::CartesianIndices_42, val, indices::NTuple{N, Index}) where N  = @throw_error MethodError ERR_CARTESIAN_SETINDEX
