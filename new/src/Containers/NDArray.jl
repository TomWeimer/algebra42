...
mutable struct NDArray{T,N} <: AbstractNDArray{T,N}
   ...
   # Default constructor
   NDArray(content::AbstractArray{T}, shape::Dims{N}, strides) where {T,N} = (
    any(d -> d < 0, tuple) && throw(DomainError("Dimensions cannot be negative"));
...
_init(data::AbstractArray, shape::Dims, T::Type) = (
...
function _fill(data::NestedArray, shape::Dims, ::Type{T}) where T
    content = similar(data, T, prod(shape))

    # The following operations enable us to obtain col major data:
    flat     = flatten(data, shape, T)
    tmp      = reshape(flat, reverse(shape)...)
    colmajor = permutedims(tmp, reshaped_dims(shape))
...
# flatten ( Numpy syntax )
function flatten(data::NestedArray{U}, sizeA::NTuple{N, Int}, ::Type{T}) where {U, N, T} 
    flattenData = Array{T, 1}(undef, prod(sizeA))

    for (i, I) in enumerate(CartesianIndices42(sizeA, order=RowOrder))
        flattenData[i] = T(_get_nested(data, I))
    end
    return flattenData
end
...
Vector{T}(data::NestedArray) where T = (
    shape = getShape(data);
    length(shape) == 1 || throw(ArgumentError("This is not a vector use vecA instead"));
    _init(data, shape, T);
)

Vector{T}(data::MultiDimArray{S, 1}) where {T, S} = _init(data, size(data), T)
...
Matrix{T}(data::NestedArray) where T  = (
    shape = getShape(data);
    length(shape) == 2 || throw(ArgumentError("This is not a matrix"));
    _init(data, shape, T);
)

Matrix{T}(data::MultiDimArray{S, 2}) where {T, S} = _init(data, size(data), T)
