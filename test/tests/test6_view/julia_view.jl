...
function test_colon_view(write_log_fn)
    # -----------------------------
    # Colon view
    # -----------------------------
...
function test_dimension_droping(write_log_fn)
    # -----------------------------
    # Dimension dropping
    # -----------------------------
    A = reshape(collect(0:15), 4, 4)
...
function test_strided_slice(write_log_fn)
...
function test_view_of_view(write_log_fn)
    A = reshape(collect(0:15), 4, 4)
...
function test_transpose_view(write_log_fn)
    B = reshape(collect(0:8), 3, 3)
...
function test_write_through_view(write_log_fn)
    B = reshape(collect(0:8), 3, 3)
...
function test_shape_and_strides_consistency(write_log_fn)
    # -----------------------------
...
function test_nested_view(write_log_fn)
...
function run_tests_julia_view(write_log_fn)
    test_basic_view(write_log_fn)
    test_colon_view(write_log_fn)
    test_dimension_droping(write_log_fn) # scalar
    test_strided_slice(write_log_fn)
    test_view_of_view(write_log_fn)
    test_fancy_indexing_copy(write_log_fn)
    test_scalar_view(write_log_fn)
    #test_transpose_view(write_log_fn)
