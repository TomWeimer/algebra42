...
# There is no remaining strides so return the previous_strides
compute_strides(previous_strides::NTuple{N, Int}, ::Dims{N}) where N = previous_strides

# Merge previous strides with the remaining ones
function compute_strides(previous_strides::NTuple{N_prev, Int}, dims::Dims{N}) where {N_prev, N}
    n_missing = N - N_prev
