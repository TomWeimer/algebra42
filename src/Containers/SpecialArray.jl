...
Base.axes(a::AbstractDiagonalArray{T,N}) where {T,N} = (
    d = diagonal_length(a); ntuple(_ -> 1:d, N)
)

# ──── Internal functions ──────────────────────────────────────────────────────────────────────── #

_getElement(a::AbstractDiagonalArray{T,N}, i::Int) where {T,N} = (
    __getElement(a, i)
)

_getElement(a::AbstractDiagonalArray{T,N}, I::AllCartesianIndex{N}) where {T,N} = (
    __getElement(a, I)
)

_getElement(a::AbstractDiagonalArray{T,N}, indices::NTuple{N,Index}) where {T,N} = (
    __getElement(a, indices)
)

# ──── getter/setter errors ────────────────────────────────────────────────────────────────────── #
            
_getElement(::AbstractDiagonalArray{T,0}) where T = (
    @throw_error ArgumentError ERR_SCALAR_DIAG
)

_setElement!(::AbstractDiagonalArray{T,0}, val) where T = (
    @throw_error ArgumentError ERR_SCALAR_DIAG
)

_setElement!(::AbstractDiagonalArray, val, i::Int) = (
    @throw_error ArgumentError ERR_NOT_STORED_VALUE
)

_setElement!(::AbstractDiagonalArray{T,N}, val, I::AllCartesianIndex{N}) where {T,N} = (
    @throw_error ArgumentError ERR_NOT_STORED_VALUE
)

_setElement!(::AbstractDiagonalArray{T,N}, val, indices::NTuple{N,Any}) where {T,N} = (
    @throw_error ArgumentError ERR_NOT_STORED_VALUE
)
...
__getElement(a::AbstractDiagonalArray{T,N}, i::Index) where {T,N} = (
    __getElement(a, CartesianIndex42(a, i))
)

# And if the array is a matrix we keep the linear index
__getElement(a::AbstractDiagonalMatrix{T}, i::Index) where T = (
    diagLength = diagonal_length(a);
    isOnDiag(a, i, diagLength) ? getDiagValue(a, i, diagLength) : zero(T)
)

__getElement(a::AbstractDiagonalArray{T,N}, idx::Indices{N}) where {T,N} = (
    dim1 = first(idx);
...
isOnDiag(::AbstractDiagonalMatrix, i::Int, diagLength::Int) = (i - 1) % (diagLength + 1) == 0

isOnDiag(::AbstractDiagonalArray{T,N}, I, dim1::Int) where {T,N} = (
...
DiagonalMatrix(size::NTuple{2,Int}, diagonal::Vararg{T,M}) where {T,M} = (
...
# ──── implements AbstractDiagonalArray ────────────────────────────────────────────────────────── #
...
_setElement!(::ZeroArray, val, i::Int) = (
    @throw_error ArgumentError ERR_NOT_STORED_VALUE
)

_setElement!(::ZeroArray{T,N}, val, I::AllCartesianIndex{N}) where {T,N} = (
    @throw_error ArgumentError ERR_NOT_STORED_VALUE
)

_setElement!(::ZeroArray{T,N}, val, indices::NTuple{N,Any}) where {T,N} = (
    @throw_error ArgumentError ERR_NOT_STORED_VALUE
)